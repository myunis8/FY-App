...
def _asignar_etiquetas(simbolos, etiquetas, ppm, avisos):
    ALIM = ("alimentacion_extractor", "alimentacion_estufa", "alimentacion_bomba",
            "toma_termotanque")
    ...
    for i, e in enumerate(etiquetas):
        if i not in usadas:
            avisos.append({"tipo": "etiqueta_sin_simbolo", "gravedad": "advertencia",
                           "texto": e.texto, "pt": [round(e.x, 1), round(e.y, 1)],
                           "detalle": "No encontré ninguna caja cerca de esta leyenda."})
    for j, s in enumerate(destinos):
...
def extraer(ruta_pdf: str, pt_por_metro: Optional[float] = None,
    ...
    virt = []
    for a, b in correcciones.get("murosVirtualesM", []):
        virt.append(((a[0] * ppm + ref.x_pdf_pt, ref.y_pdf_pt - a[1] * ppm),
                     (b[0] * ppm + ref.x_pdf_pt, ref.y_pdf_pt - b[1] * ppm)))
    ambientes, lab, S = amb.detectar(caminos, ppm, rect, ref.envolvente_pdf_pt, virt)
    ...
    def agregar(clase, x, y, extra):
        ...
        amb_id, conf_amb = amb.ambiente_de((x, y), ambientes, lab, S, ppm)
        base = {
            "id": eid,
        ...
        base.update(extra)
        base.setdefault("confianza", {})["ambiente"] = conf_amb
    ...
    for s in simbolos:
        ...
        if s.clase in CLASES_LUZ:
            ...
            if letra is None and et and et.subtipo in (
                    "alimentacion_extractor", "alimentacion_estufa",
                    "alimentacion_bomba", "toma_termotanque"):
                agregar("alimentacion", s.x, s.y, {
                    "tipo": "alimentacion", "subtipo": et.subtipo,
    ...
    # ---- ambientes ----
    amb.nombrar(ambientes, [{"ambiente_id": e["ambienteId"], "subtipo": e.get("subtipo")}
                            for e in elementos])
    nombres = {r["ambienteId"]: r["nombre"] for r in correcciones.get("ambientes", [])
               if r.get("ambienteId")}
    lista_amb = []
    for a in ambientes:
        poly = [[round(v, 3) for v in a_m(x, y)] for x, y in a.poligono_pt]
        xs = [p[0] for p in poly]; ys = [p[1] for p in poly]
        nombre = nombres.get(a.id, a.nombre)
        lista_amb.append({
            "id": a.id, "nombre": nombre,
            "origenNombre": "usuario" if a.id in nombres else a.origen_nombre,
            "confianzaNombre": 1.0 if a.id in nombres else a.confianza_nombre,
            "areaM2": a.area_m2, "esExterior": a.es_exterior,
            "bboxM": [round(min(xs), 3), round(min(ys), 3),
                      round(max(xs), 3), round(max(ys), 3)],
            "poligonoM": poly, "notas": a.notas})
        if nombre == "desconocido" and not a.es_exterior:
            avisos.append({"tipo": "ambiente_sin_nombre", "gravedad": "advertencia",
                           "ambienteId": a.id,
                           "detalle": f"No pude nombrar este ambiente de {a.area_m2} m²."})

    sin_ambiente = sum(1 for e in elementos if not e["ambienteId"])
    if sin_ambiente:
        avisos.append({"tipo": "elementos_sin_ambiente", "gravedad": "advertencia",
                       "cantidad": sin_ambiente,
                       "detalle": f"{sin_ambiente} elementos quedaron fuera de todo ambiente."})
    ...
    return {
        ...
        "resumen": {
            ...
            "alimentaciones": resumen.get("alimentacion", 0),
            "sinReconocer": resumen.get("desconocido", 0),
            "ambientes": len([a for a in lista_amb if not a["esExterior"]]),
