...
Doble clic en `build.bat`. Queda en `dist\ObrasElectricas.exe`, un archivo
...
| Qué | Dónde |
|---|---|
| Obras | `%LOCALAPPDATA%\ObrasElectricas\obras\<id>\` |
| Configuración y token | `%APPDATA%\ObrasElectricas\config.json` |
...
1. **Extractor** — lee el PDF y llena `plano`, `ambientes`, `elementos`.
2. **Circuitos** — agrupa en IUG/TUG/TUE y llena `circuitos`, `tableros`.
